...
- 评估协议：leave-one-out；过滤 seen 行为（训练/验证集中已交互的物品）后进行全物品打分与 Top-K 选择；指标定义如下：
  - Recall@K：命中即为 1，否则为 0；对用户平均。
  - NDCG@K：命中位置的折损增益，位置越靠前得分越高。
  - HitRate@K：与 Recall@K 一致（单一正例设定）。

### 运行命令（Baseline 与 SGL）
```bash
# Baseline（LightGCN）训练与评估
...
![Baseline 曲线图](img/baseline_curves.png)
