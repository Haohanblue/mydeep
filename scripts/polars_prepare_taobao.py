...
def leave_one_out_split(df: pl.DataFrame) -> Tuple[pl.DataFrame, pl.DataFrame, pl.DataFrame]:
    ...
    - train: rn <= cnt-3
    """
    df = df.sort(["user_id", "timestamp"]).with_row_index("row_idx")

    df = df.with_columns(
        [
            (pl.col("row_idx") - pl.col("row_idx").min().over("user_id")).alias("rn"),
            pl.len().over("user_id").alias("cnt"),
        ]
    )
    ...
    train = train.drop(["rn", "cnt", "row_idx"])
    valid = valid.drop(["rn", "cnt", "row_idx"])
    test = test.drop(["rn", "cnt", "row_idx"])
